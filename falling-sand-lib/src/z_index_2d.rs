...
/// z index struct to store 2D coordinates as inherently interlaced bits.
pub struct ZIndex{
    index: u64
}
...
impl ZIndex{
    pub fn from_coords(x: u32, y: u32) -> Self{
        let index = unsafe{
            _pdep_u64(u64::from(x), X_BITS) | _pdep_u64(u64::from(y), Y_BITS)
        };
        Self::new(index)
    }
    ...
    pub fn x(&self) -> u64{
        unsafe {
            _pext_u64(self.index, X_BITS)
        }
    }
    pub fn y(&self) -> u64{
        unsafe {
            _pext_u64(self.index, Y_BITS)
        }
    }
